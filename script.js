    ...
    function fetchUser(username) {
      fetch(`https://api.github.com/users/${username}`)
        ...
        .then(user => {
          ...
          document.querySelector('.repo').innerHTML = `<img src="./comps/link.png" class="licon" alt=loc><a href="${user.html_url}" target="_blank" class="textblack">${user.html_url}</a>`;
          if (user.bio) document.querySelector('.bio').innerHTML = `${user.bio}`;
          if (user.location) document.querySelector('.location').innerHTML = `<img src="./comps/location.png" class="licon" alt=loc>  ${user.location}`;
          ...
          const repos = data.map((repo)=>{
            ...
            const desc= repo.description ? `<p>${repo.description}</p>` : '';
                return `<div class="card flex-col">
                          <a class="textwhite" href="${repo.html_url}">${repo.name}</a>
                          <a class="textblue">${desc}</a>
